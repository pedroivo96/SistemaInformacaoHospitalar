...
<html lang="en">
  <head>
    ...
    <title>Resultados dos procedimentos</title>
		...
		<div class="col-md-9">
			<?php
				...
				if(!empty($_POST)){
					...
					$sql = 'SELECT nomeprocedimento, anotacoesopcionais FROM procedimentos WHERE id = :idprocedimento';
					$stmt = $conn->prepare($sql);
					$stmt->bindValue(':idprocedimento', $idprocedimento);
					$stmt->execute();
					$count = $stmt->rowCount();
					...
					if($count > 0){
						...
						foreach($result as $row){
							...
							?>
							<dl class="row">
								<dt class="col-sm-4">Nome do procedimento</dt>
								<dd class="col-sm-8"><?php echo $nomeprocedimento; ?></dd>
							...
							$sql1 = 'SELECT nomeimagem FROM resultadosprocedimentos WHERE id = :idprocedimento';
